...
export function getTableRepresentation({ connectionName, schemaName, tableName, prefix = "", value, perPage = 1000, visited = [], onReady, onTableLoaded }: TTableRepresentation): { representation: TRepresentation; done: boolean; } {
    const struct = databaseStructureStore.getState().data?.find(t => t.table === tableName && t.schema === schemaName);
    ...
    tableStore.getState().initialize({ query: { connectionName, schema: schemaName, table: tableName, page: 1, perPage } });

    const onTableLoadedInternal = () => {
        onTableLoaded?.();
        if (onReady) {
            const res = process();
            console.log("onTableLoadedInternal", tableName, res.representation, res.done);
            if (res.done) {
    ...
    const process = () => {
        ...
        let representation: TRepresentation = { schemaName, tableName, identifier: value, representation: [], dependencies: [] };
    ...
    if (!tableStore.getState().done) {
        ...
        return { representation: { schemaName, tableName, identifier: value, representation: [], dependencies: [] }, done: false };
    }
